...
export interface ChromeSegment { a: ChromePoint; b: ChromePoint; mid: ChromePoint; }
...
export function selectedConnectorChrome(frame: PageDisplayList, page: PageSnapshot, selection: ChromeSelection): SelectedConnectorChrome | null {
  ...
  for (const run of runs) for (let index = 1; index < run.length; index++) {
    const a = run[index - 1], b = run[index];
    if (a.x === b.x && a.y === b.y) continue;
    segments.push({ a, b, mid: { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 } });
  }
  return { chrome, start: vertices[0], end: vertices[vertices.length - 1], segments, draggable: runs.length === 1 ? runs[0] : null };
}

function findShapePath(primitives: readonly PagePrimitive[], id: string, depth = 0): { commands: readonly GeometryPathCommand[]; transform?: Affine } | null {
  if (depth >= 256) return null;
  for (const primitive of primitives) {
    if (primitive.kind === 'shape' && primitive.id === id) return { commands: primitive.path, transform: primitive.transform };
...
/** Index of the segment dot under the pointer, if any. */
export function hitSegmentDot(selected: SelectedConnectorChrome, point: ChromePoint, tolerance: number): number | null {
  let best: number | null = null;
  let nearest = tolerance;
  for (let index = 0; index < selected.segments.length; index++) {
    const distance = Math.hypot(selected.segments[index].mid.x - point.x, selected.segments[index].mid.y - point.y);
