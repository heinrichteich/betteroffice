...
export interface DragStart { canvas: ModelPoint; model: ModelPoint; resize: boolean; handle?: ResizeHandle; rotate?: boolean; pin: ModelPoint; locPin?: ModelPoint; locPinFraction?: { x?: boolean; y?: boolean }; size: { width: number; height: number }; parentTransforms?: readonly Affine[]; angle?: number; flipX?: boolean; flipY?: boolean; pointerId?: number; startX?: number; startY?: number; thresholdPassed?: boolean; }
...
export const resolveDragGeometry = (start: DragStart, release: ModelPoint): { x: number; y: number; width: number; height: number } => {
  ...
  if (start.handle) {
    ...
    const newLocPinX = (start.locPinFraction?.x ?? start.locPin?.x === undefined) ? fx * next.width : locPin.x;
    const newLocPinY = (start.locPinFraction?.y ?? start.locPin?.y === undefined) ? fy * next.height : locPin.y;
...
export const previewOutline = (start: DragStart, release: ModelPoint, paintTransform: Affine, snap = false): ModelPoint[] => {
  ...
  const effFx = (start.locPinFraction?.x ?? start.locPin?.x === undefined) ? oldFx : (geometry.width > 0 ? locPin.x / geometry.width : 0.5);
  const effFy = (start.locPinFraction?.y ?? start.locPin?.y === undefined) ? oldFy : (geometry.height > 0 ? locPin.y / geometry.height : 0.5);
