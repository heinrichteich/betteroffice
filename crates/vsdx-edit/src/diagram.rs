...
use crate::{
    CONNECTS, CellFormulaReceipt, CellSnapshot, ConnectedShapeReceipt, ConnectorGlue,
    DiagramSession, DiagramSnapshot, EditCtx, EditError, EditResult, META, PAGES, PAGE_ORDER,
    PageSnapshot, SHEETS, STORIES, ShapeDraft, ShapeReceipt, ShapeSnapshot, ShapeTreeDraft,
    ShapeTreeGlue, TextReceipt,
};
...
fn materialize_snapshot(
    ...
    for page in &snapshot.pages {
        ...
        connect::materialize_page_glue(sheet, page, glue, original_shape_ids);
    }
    Ok(())
}
...
pub(super) fn glue_text_valid(value: &str) -> bool {    let limits = ParseLimits::default();
...
/// Evaluated OneD decides alone; free-end connector drafts only need the gate.
...
fn insert_shape_in(
    ...
    let id = allocate_shape_id(txn, sheets, page_id, client_id);
...
impl DiagramSession {
    ...
    pub fn add_shape_tree(
        ...
        for node in &nodes {
            ...
            match sheets.get(&txn, source_id.as_str()) {
                ...
                _ => {
                    ...
                    missing.get_or_insert_with(|| source_id.clone());
                }
            }
        }
        if found > 0 && found != nodes.len() {
        ...
        for glue in &draft.glue {
            let endpoint = GlueEndpoint::parse(glue.endpoint.as_str()).ok_or_else(|| {
                EditError::InvalidState("connector glue has no endpoint".to_owned())
            })?;
            if !valid_glue_target(&glue.to_cell) || !glue_text_valid(&glue.to_cell) {
                return Err(EditError::InvalidState(
    ...
    pub fn add_connected_shape(
        ...
        if !draft_is_one_d(connector_draft) {
            return Err(EditError::InvalidState(
                "connector draft must describe a 1D shape".to_owned(),
            ));
        }
        ...
        let to_cell = normalized_glue_target(to_cell).ok_or_else(|| {
            EditError::InvalidState("connector glue needs a valid target cell".to_owned())
        })?;
        ...
        let sheets = txn
            .get_map(SHEETS)
            .ok_or_else(|| EditError::InvalidState("missing sheets map".to_owned()))?;
        if sheets.len(&txn) as usize + 1 >= ParseLimits::default().max_shapes {
            return Err(EditError::InvalidState(
                "shape count exceeds maximum".to_owned(),
            ));
        }
        ...
        let allocated = materialized_source_ids(&sheets, &txn, &order, source_bound)?;
        let largest = allocated.values().copied().max().unwrap_or(source_bound);
        largest.checked_add(2).ok_or_else(|| {
            EditError::InvalidState("cannot allocate a materialized source ID".to_owned())
        })?;
        let source = match sheets.get(&txn, from.shape_id.as_str()) {
            Some(Out::YMap(shape)) => shape,
        ...
        let (shape_id, shape_index) = insert_shape_in(
            &mut txn,
            &sheets,
            &order,
            page_id,
        ...
        let (connector_id, connector_index) = insert_shape_in(
            &mut txn,
            &sheets,
            &order,
            page_id,
        ...
        for (endpoint, target, cell) in [
            ...
            (GlueEndpoint::End, shape_id.as_str(), to_cell.as_str()),
        ] {
            let key = glue_key(connector_id.as_str(), endpoint);
            let entry = connects.insert(&mut txn, key.as_str(), MapPrelim::default());
            entry.insert(&mut txn, "id", key.as_str());
            entry.insert(&mut txn, "pageId", page_id);
            entry.insert(&mut txn, "connectorId", connector_id.as_str());
            entry.insert(&mut txn, "endpoint", endpoint.name());
            entry.insert(&mut txn, "targetId", target);
            entry.insert(&mut txn, "toCell", cell);
        }
        Ok(ConnectedShapeReceipt {
            shape: ShapeReceipt {
                page_id: page_id.to_owned(),
                shape_id,
                from_index: None,
                to_index: Some(shape_index),
            },
            connector: ShapeReceipt {
                page_id: page_id.to_owned(),
                shape_id: connector_id,
                from_index: None,
                to_index: Some(connector_index),
            },
        })
    }
