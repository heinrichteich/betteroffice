...
use crate::{
    ...
    ShapeDraft, ShapeReceipt, ShapeSnapshot, ShapeTreeDraft, ShapeTreeGlue, TextReceipt,
};
...
fn seed_shape(
    ...
    let text = resolver
        ...
        .map_err(|error| EditError::InvalidState(error.to_string()))?;
    stories.insert(txn, id, plain_text(&text, resolved));
...
fn plain_text(
    ...
    for token in tokens {
        match token {
            ...
            vsdx_resolve::ResolvedTextToken::Field { index, properties } => {
...
fn validate_remote_text(before: &Doc, staged: &Doc) -> EditResult<()> {
    ...
    for (shape_id, staged_shape) in staged_sheets.iter(&staged_txn) {
        ...
        let Some(Out::YMap(before_shape)) = before_sheets.get(&before_txn, shape_id) else {
            if let Some(text) = match staged_txn
                .get_map(STORIES)
                .as_ref()
                .and_then(|stories| stories.get(&staged_txn, shape_id))
            {
        ...
        let before_text = story_text(&before_txn, shape_id);
...
fn semantic_text_edits_in(
    ...
    for (shape_id, shape) in sheets.iter(&txn) {
        ...
        let tokens = resolver
            .resolve_text_in_context(original, page_sheet, &resolved)
            .map_err(|error| EditError::InvalidState(error.to_string()))?;
        if plain_text(&tokens, &resolved) == current {
            continue;
        }
...
fn serialize_doc(doc: &Doc) -> EditResult<Vec<u8>> {
    ...
    if !structural.is_empty() || !text_edits.is_empty() {
        let staged =
            vsdx_parse::parse_vsdx(&bytes).map_err(|error| EditError::Parse(error.to_string()))?;
        bytes = vsdx_parse::save_semantic_cell_edits(&staged, &cell_edits)
            .map_err(|error| EditError::Parse(error.to_string()))?;
    }
