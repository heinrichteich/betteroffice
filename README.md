...
Visio drawings (`.vsdx`) and templates (`.vstx`) open in the source-available VSDX core, which is not yet published. Stencils (`.vssx`) and macro-enabled files are not supported. Pages export to SVG (vector, selectable text) and PNG (scaled raster).
